...
def main(args):
    ...
    for param in model.decoder.prompt_linear_verbalizer.parameters():
        # print("aaaapppp:", param)
        verbalizers.append(param.detach().cpu().numpy())
    print("verbalizers shaep", np.array(verbalizers).shape)
    np.savez("1_IC_hubert_weights.npz", data = verbalizers)
    exit()
...
def cli_main():
    ...
    input_args = [
        ...
        "--user-dir=./prompt_lm_module_verbalizer",
